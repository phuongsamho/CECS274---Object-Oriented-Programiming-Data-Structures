public class Bank
{
   ...
   public void AddAccount(int accountID, String customerName) throws Exception
   {
	...
	myTree.put(myAccountID, myCustomer);
   };
   ...
   {
	AccountID myAccountID = new AccountID(accountID);
	Customer myCustomer = myTree.get(myAccountID);
   ...
   public void Report()
   {
	java.util.Set<AccountID>keys = myTree.keySet();
	java.util.Iterator<AccountID> itr = keys.iterator();
	while (itr.hasNext())
	{
		Customer data = myTree.get(itr.next());
		data.Visit();
	}
   };

   // YOU Add the tree here
   java.util.TreeMap<AccountID, Customer> myTree = new java.util.TreeMap<AccountID, Customer>(); 	
};
